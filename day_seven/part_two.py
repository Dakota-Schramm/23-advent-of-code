...
def calculate_first_ordering(hand_winning):
  ...
  if wildcards in [4, 5]:     
    # print(f"***{wildcards}")
    return FIRST_ORDERING[0]

  vals = list(card_counts.values())
  if wildcards == 3:
    # print("***3")
    if 2 in vals:             return FIRST_ORDERING[0]
    else:                     return FIRST_ORDERING[1]
  elif wildcards == 2:
    # print("***2")
    if 3 in vals:             return FIRST_ORDERING[0]
    elif vals.count(2) == 2:  return FIRST_ORDERING[1]
    else:                     return FIRST_ORDERING[3]
  elif wildcards == 1:
    if 4 in vals:             return FIRST_ORDERING[0]
    elif 3 in vals:           return FIRST_ORDERING[1]
    elif 2 in vals:           return FIRST_ORDERING[3]
