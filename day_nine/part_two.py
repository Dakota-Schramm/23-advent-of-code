def find_next_value_of_history(history):
  ...
  for i in reversed(range(n)):
    if i == n-1:
      history_sequence[i] = [0, *history_sequence[i]]
    else:
      diff      = history_sequence[ i + 1 ][0]
      new_value = history_sequence[ i ][0] - diff
      history_sequence[ i ] = [ new_value, *history_sequence[i] ]
  
  print(history_sequence)
  return history_sequence[0][0]
